...
queue_url = client.get_queue_url(QueueName=ts_config.get('aws.sqs.stream-segment--analyze.name'))['QueueUrl']
queue = resource.Queue(queue_url)

def send_message(payload):
    logger.info("send_message | start", payload=payload)
    r = queue.send_message(MessageBody=json.dumps(payload))
