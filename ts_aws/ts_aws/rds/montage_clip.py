...
def save_montage_clips(montage_clips):
    ...
    values = ', '.join(list(map(lambda ss: str(tuple([('NULL' if ss[column.key] is None else ss[column.key]) for column in ts_model.MontageClip.__table__.columns])), montage_clips)))
    query = f"REPLACE INTO {ts_model.MontageClip.__tablename__} ({', '.join(columns)}) VALUES {values};"
    logger.info("save_montage_clips | query", query=query)
